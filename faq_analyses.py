...
# Дополнительные 1000 вопросов: вариации и синонимы (итого 1400+ записей)
def _expand_faq():
    out = list(FAQ)
    for item in list(FAQ):
        ...
        if "норма" in q:
            out.append({"q": q.replace("норма", "нормы").replace("у женщин", "для женщин").replace("у мужчин", "для мужчин"), "a": a})
    while len(out) < 1400:
        out.extend(FAQ)
    return out[:1400]
...
# Синонимы и аббревиатуры для точного совпадения темы (показатель должен быть в вопросе FAQ)
SYNONYMS = {
    "ттг": {"ттг", "тиреотропный", "тиротропин"},
    "т3": {"т3", "трийодтиронин"},
    "т4": {"т4", "тироксин"},
    "гемоглобин": {"гемоглобин", "гемоглобин", "hb"},
...
def _normalize(text: str) -> List[str]:
    ...
    words = text.split()
    stop = {"что", "это", "значит", "как", "почему", "какой", "какая", "какие", "причины", "причина", "у", "в", "на", "при", "по", "для", "из", "и", "или", "с", "без", "не", "нет", "да", "как", "так", "же", "ли", "нужно", "надо", "можно", "делать", "значит", "означает", "быть", "есть", "анализ", "анализе", "анализов", "крови", "моче", "мочи"}
    return [w for w in words if len(w) > 1 and w not in stop]


def _expand_query_tokens(tokens: set) -> set:
    """Расширяем токены запроса синонимами для сопоставления с вопросами FAQ."""
    out = set(tokens)
    for t in tokens:
        if t in SYNONYMS:
            out |= SYNONYMS[t]
...
MIN_SCORE = 3.0
# Минимум совпадающих токенов в вопросе FAQ (иначе ответ может быть не по теме)
...
def search_faq(query: str, top_k: int = 3) -> List[Tuple[str, str, float]]:
    ...
    Возвращает только релевантные ответы: обязательное совпадение в вопросе, порог по score, дедупликация.
    """
    ...
    for item in FAQ_LIST:
        ...
        # Обязательно хотя бы один значимый термин из запроса должен быть в вопросе FAQ
        if len(match_q) < MIN_MATCH_IN_QUESTION:
            continue
        # Оценка: совпадения в вопросе важнее, в ответе — доп. релевантность
    ...
    # Дедупликация по тексту ответа — один ответ один раз (с лучшим вопросом по score)
    by_answer = {}
